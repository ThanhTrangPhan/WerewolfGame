...
public class ClientMainFrm extends JFrame implements ActionListener{
    ...
    public ClientMainFrm(){
        ...
        mnPlayer.add(mniLogin);

        mniGroup = new JMenuItem("Group");
        mniGroup.addActionListener(this);
        mnPlayer.add(mniGroup);
        mnbMain.add(mnPlayer);
        this.setJMenuBar(mnbMain);
        mniLogin.setEnabled(false);
        mniGroup.setEnabled(false);
    ...
    public void actionPerformed(ActionEvent ae) {
        ...
        if(ae.getSource() instanceof JButton) {
            ...
            if(mni.equals(mniLogin)) {// login function
                ...
                clv.setVisible(true);
            }else if(mni.equals(mniGroup)) {// Search client to edit
                for(ObjectWrapper func: myControl.getActiveFunction())
                    if(func.getData() instanceof GroupViewFrm) {
                        ((GroupViewFrm)func.getData()).setVisible(true);
                        return;
                    }
                GroupViewFrm scv = new GroupViewFrm(myControl);
                scv.setVisible(true);
            }
        }
    }
